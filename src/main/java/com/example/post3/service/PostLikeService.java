...
public class PostLikeService {
    ...
    public ResponseEntity<String> addpostlike(Long postId, UserDetailsImpl userDetails) {
        ...
        if (postLikeRepository.findByUserAndPost(user, post).isPresent()) {
            Boolean like = postLikeRepository.findByUserAndPost(user, post).get().getLike();
            if (like) { // 가져온 like 값이 true 라면
                ...
                postLike.setLike(true); // 하면 false 값을 true로 변경함
                // 하고 다시 save 해줄건데 왜 안되냐 ...?
            ...
            postLike.setLike(true); // true 값으로 넣어서 저장할 것
            postLikeRepository.save(postLike);
    ...
    public ResponseEntity<String> cancelpostlike(Long postId, UserDetailsImpl userDetails) {
        Post post = findPost(postId);
        User user = userDetails.getUser();
        ...
        if (postLikeRepository.findByUserAndPost(user, post).isPresent()) {
            ...
            if (postLike.getLike()) { // 가져온 like 값이 true 라면
                // false로 수정하기
